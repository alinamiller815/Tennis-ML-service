...
def make_prediction(
    ...
    """Создание предсказания исхода теннисного матча"""
    try:
        # Преобразуем входные данные в словарь
        input_data = prediction_data.dict()
        model_id = input_data.pop("model_id", None)
        ...
        if "features_used" in output_data:
            print(f"Модель использовала {output_data['total_features']} признаков: {output_data['features_used']}")
